...
public class BusinessLogicLayerTemplate_5G_CSHARP : ITemplate
{
    ...
    //bool ITemplate.Execute(ref MyMeta.IDatabase db)
    {
        ...
        try
        {
            ...
            BuildDataField();
            if (NetVersion == NetVersionEnum.Net40)
                BuildDataHandlerBaseNET40();
            if (NetVersion == NetVersionEnum.Net45)
    ...
    private void BuildBusinessRelations(MyMeta.IDatabase db, string _namespace)
    {
        ...
        foreach (MyMeta.ITable entity in db.Tables)
        {
            if (entity.Selected)
            {
                ...
                output.AppendLine("            public WhereCollection Where { get; set; }");
                output.AppendLine("            public new OrderByCollection OrderByParameter { get; set; }");
                output.AppendLine("            public new GroupByCollection GroupByParameter { get; set; }");
                output.AppendLine("            public new AggregateCollection AggregateParameter { get; set; }");
                ...
                output.AppendLine("                Where = new WhereCollection();");
                output.AppendLine("                OrderByParameter = new OrderByCollection();");
                output.AppendLine("                GroupByParameter = new GroupByCollection();");
                output.AppendLine("            }");
                ...
                output.AppendLine("                     this.whereParameter.Add(conjunction, Enum.GetName(typeof(ColumnEnum), column), operand, value);");
                output.AppendLine("                 }");
                output.AppendLine("                 public void Clear()"); // MODIF- 2-03-2016 
                output.AppendLine("                 {");
                output.AppendLine("                     this.whereParameter.Clear();");
                output.AppendLine("                 }");
                ...
                output.AppendLine("                         return this.whereParameter.Count;");
                output.AppendLine("                     }");
                output.AppendLine("                 }"); // MODIF- 2-03-2016 
                output.AppendLine("            }");
                output.AppendLine("            public class OrderByCollection : OrderByParameter {");
                output.AppendLine("                 internal OrderByParameter orderByParameter = new OrderByParameter();");
                output.AppendLine("                 public void Add(ColumnEnum column, " + _namespace + ".sqlEnum.DirEnum direction = " + _namespace + ".sqlEnum.DirEnum.ASC)");
        ...
        foreach (MyMeta.ITable entity in db.Tables)
        {
            if (entity.Selected)
            {
                ...
                foreach (MyMeta.IColumn column in entity.Columns)
                {
                    ...
                    if (column.Name.Equals(_generationProject.Entity_Display_Name))
    ...
    private void BuildEntitiesProgrammability(MyMeta.IDatabase db, string _namespace)
    {
        ...
        foreach (MyMeta.IProcedure entity in db.Procedures)
        {
            if (entity.Selected)
            {
                ...
                {
                    output.AppendLine("					public const string " + column.Name + " = " + System.Convert.ToChar(34) + column.Name + System.Convert.ToChar(34) + ";");
                }
                output.AppendLine("				}");
                ...
                foreach (MyMeta.IResultColumn column in entity.ResultColumns)
                {
                    ...
                    if (column.Name.Equals("Name"))
                        output.AppendLine("             [PropertyAttribute(PropertyAttribute.PropertyAttributeEnum.Display)] //Id Display Default");
                    //output.AppendLine("             public " + column.DataTypeName.Split('.')[1].ToString() + " " + column.Name + " { get; set; }");
    ...
    private void BuildEntitiesRelations(MyMeta.IDatabase db, string _namespace, string childrenPrefix)
    {
        ...
        foreach (MyMeta.ITable entity in db.Tables)
        {
            if (entity.Selected)
            {
                ...
                string formattedEntityName = GetFormattedEntityName(entity.Name);
                if (entity.Name.Equals("Usuarios"))
                ...
                foreach (MyMeta.IColumn column in entity.Columns)
                {
                    ...
                    if (column.IsInForeignKey)
                        ...
                        {
                            if (!output.ToString().Contains("             internal " + column.LanguageType + " _" + formattedEntityName + "_" + fk.PrimaryTable.Name + "_" + GetFormattedEntityName(column.Name) + " { get; set; } // Value holder for " + GetFormattedEntityName(column.Name)))
                                output.AppendLine("             internal " + column.LanguageType + " _" + formattedEntityName + "_" + fk.PrimaryTable.Name + "_" + GetFormattedEntityName(column.Name) + " { get; set; } // Value holder for " + GetFormattedEntityName(column.Name));
                        }
                    ...
                    if (column.IsInForeignKey && !column.IsInPrimaryKey)
                    {
                        ...
                        String refParams = string.Empty;
                        foreach (MyMeta.IForeignKey fk in column.ForeignKeys)
                            foreach (MyMeta.IColumn refFkTableColumn in fk.PrimaryTable.Columns)
                                if (refFkTableColumn.IsInForeignKey && refFkTableColumn.IsInPrimaryKey)
                                {
                                    //refParams += "this._" + formattedEntityName + "_" + fk.PrimaryTable.Name + "_" + GetFormattedEntityName(refFkTableColumn.Name) + ",";
                                    refParams += "this._" + formattedEntityName + "_" + fk.PrimaryTable.Name + "_" + GetFormattedEntityName(column.Name) + ",";
                                }
                        output.AppendLine("             [PropertyAttribute(PropertyAttribute.PropertyAttributeEnum.Fk)] //Is Foreign Key");
                        output.AppendLine("             [PropertyAttributeForeignKeyObjectName(" + System.Convert.ToChar(34) + column.ForeignKeys[0].PrimaryTable.Name + System.Convert.ToChar(34) + ")]// Object name in Database");
                        //output.AppendLine("             public " + _namespace + ".Entities.Relations." + GetSchemaName(entity.Schema) + "." + column.ForeignKeys[0].PrimaryTable.Name + " " + column.Name + " { get; set; }");
                        ...
                        output.AppendLine("             {");
                        //output.AppendLine("                 get { return new " + _namespace + ".Entities.Relations." + GetSchemaName(entity.Schema) + "." + column.ForeignKeys[0].PrimaryTable.Name + "();}");
                        output.AppendLine("                 get {");
                        output.AppendLine("                     if (" + GetFormattedEntityName(column.Name) + "_ == null || " + GetFormattedEntityName(column.Name) + "_." + GetFormattedEntityName(GetPrimaryKeyName(column.ForeignKeys[0].PrimaryTable)) + " != _" + formattedEntityName + "_" + column.ForeignKeys[0].PrimaryTable.Name + "_" + GetFormattedEntityName(column.Name) + ")");
                        output.AppendLine("                         {");
                        output.AppendLine("                             " + GetFormattedEntityName(column.Name) + " = new " + _namespace + ".Business.Relations." + GetSchemaName(column.ForeignKeys[0].PrimaryTable.Schema) + "." + column.ForeignKeys[0].PrimaryTable.Name + "().Items(" + refParams.Substring(0, refParams.Length - 1) + ").FirstOrDefault();");
                        output.AppendLine("                         }");
                ...
                foreach (MappedStoredProcedure.ResultSet mappedProcedureResultset in mappedStoredProcedure.ResultSets)
                {
                    ...
                    foreach (MappedStoredProcedure.ProcedureField procedureField in mappedProcedureResultset.ProcedureFields)
                    {
                        ...
                        if (procedureField.Name.Equals("Name"))
                            output.AppendLine("             [PropertyAttribute(PropertyAttribute.PropertyAttributeEnum.Display)] //Id Display Default");
                        output.AppendLine("             public " + procedureField.DataType + " " + procedureField.Name + " { get; set; }");
                    }
                    output.AppendLine("     }");
                }
                output.AppendLine("     }");
    ...
    private void BuildBusinessMappedProcedures()
    {
        ...
        foreach (MappedStoredProcedure mappedStoredProcedure in _generationProject.MappedStoredProcedures)
        {
            ...
            if (mappedStoredProcedure.ResultSets.Count > 0)
            {
                ...
                {
                    definedColumnList += parameter.DataType.ToString() + " " + parameter.Name + ",";
    ...
    private static void GetHeaderInfo(System.Text.StringBuilder output)
    {
    ...
    private void BuildDataHandlerBaseNET40()
    {
        ...
        //output.AppendLine("                }");
        //output.AppendLine("            }");
        ////output.AppendLine("            //_objectNameTypeAttribute = (DataItemAttributeObjectName)(((System.Reflection.MemberInfo)(_dataItemType))).GetCustomAttributes(false)[0];");
        ////output.AppendLine("            //_objectSchemaTypeAttribute = (DataItemAttributeSchemaName)(((System.Reflection.MemberInfo)(_dataItemType))).GetCustomAttributes(false)[1];");
        //
        ...
        //output.AppendLine("                    {");
        //output.AppendLine("                        index = i;");
        //output.AppendLine("                        break;");
        //output.AppendLine("                    }");
        //output.AppendLine("                }");
        //
        //output.AppendLine("                if (customAttributesCount > 0)");
        //output.AppendLine("                {");
        //output.AppendLine("                    if (info.GetCustomAttributes(false)[index].GetType().Name.Equals(Constants.PROPERTYATTRIBUTE))");
        //output.AppendLine("                     {");
        //output.AppendLine("                         switch (((PropertyAttribute)(info.GetCustomAttributes(false)[index]))._propertyAttribute)");
        //output.AppendLine("                         {");
        ...
        //output.AppendLine("                     if (info.GetCustomAttributes(false)[index].GetType().Name.Equals(Constants.DATAITEMATTRIBUTEFIELDNAME))");
        //output.AppendLine("                     {");
        //output.AppendLine("                         fieldName = ((DataItemAttributeFieldName)(info.GetCustomAttributes(false)[index])).FieldName;");
        //output.AppendLine("                         fieldFrameworkName = ((DataItemAttributeFieldName)(info.GetCustomAttributes(false)[index])).FieldFrameworkName;");
        //output.AppendLine("                     }");
        //output.AppendLine("                 }");
        ...
        output.AppendLine("        }");
        output.AppendLine("    }");
        output.AppendLine("    protected IDbCommand GetCommand(string query, IDbConnection connection, IDbTransaction transaction)");
        output.AppendLine("    {");
        output.AppendLine("        getAssembliesNames();");
        ...
        output.AppendLine("        {");
        output.AppendLine("             _adoNetAssemblyName = ConfigurationManager.AppSettings[Constants.ADONETASSEMBLYNAME];");
        output.AppendLine("             _adoNetConnectionTypeName = ConfigurationManager.AppSettings[Constants.ADONETCONNECTIONTYPENAME];");
        output.AppendLine("             _commandTimeout = Convert.ToInt32(ConfigurationManager.AppSettings[Constants.ADONETCOMMANDTIMEOUT].ToString());");
        output.AppendLine("             _parameterPrefix = ConfigurationManager.AppSettings[Constants.PARAMETERPREFIX].ToString();");
        output.AppendLine("        }");
        output.AppendLine("    }");
    ...
    private void BuildDataHandler()
    {
        ...
        if(_generationProject.MultiQuery)
            output.AppendLine("            _commandText +=  Configuration.PkFunction;");

        output.AppendLine("            result = ExecuteNonQuery(_commandText, _transaction,true);");

        if (!_generationProject.MultiQuery)
            output.AppendLine("            ExecuteNonQuery(Configuration.PkFunction, _transaction, true);");
        ...
        output.AppendLine("                foreach (PropertyInfo info in properties)");
        output.AppendLine("                {");
        //output.AppendLine("                    if (info.Name.Equals(pkName))");
    ...
    private void BuildCore()
    {
        ...
        output.AppendLine("namespace Propato");
        output.AppendLine("{");
    ...
    private void BuildParameterBase(string _namespace)
    {
        ...
        output.AppendLine("            {");
        //output.AppendLine("                string c = values.GetType().ToString();");
        //output.AppendLine("                switch (c)");
        output.AppendLine("                if(values.GetType().ToString().StartsWith(Constants.SYSTEM_COLLECTIONS)) ");
        output.AppendLine("                {");
        //output.AppendLine("                    case Constants.SYSTEM_OBJECT_ARRAY:");
        //output.AppendLine("                    case Constants.SYSTEM_COLLECTIONS_ARRAYLIST:");
    ...
    private void BuildProcedureDataHandler()
    {
        ...
        output.AppendLine("    /// <returns></returns>");
        output.AppendLine("    internal virtual List<IDataItem> Items(params object[] parameters)");
        output.AppendLine("    {");
        output.AppendLine("        try");
        output.AppendLine("        {");
        output.AppendLine("            _itemList.Clear();");
        output.AppendLine("            _commandText = Constants.SQL_EXEC + GetFullDataEntityName();");
        output.AppendLine("            for (int i = 1; i < parameters.Length + 1; i++)");
        output.AppendLine("            {");
        output.AppendLine("                _commandText += base._parameterPrefix + i.ToString() + " + System.Convert.ToChar(34) + "," + System.Convert.ToChar(34) + ";");
        output.AppendLine("                _parameterizedValues.Add(new ParameterItemValue(base._parameterPrefix + i.ToString(), parameters[i - 1]));");
        output.AppendLine("            }");
        output.AppendLine("            if (parameters.Length > 0)");
        output.AppendLine("                _commandText = _commandText.Substring(0, _commandText.Length - 1);");
        output.AppendLine("            _datareader = ExecuteReader(_commandText, _transaction);");
        output.AppendLine("            _itemList = MapDataReaderToDataItem(_datareader, _dataItem);");
        output.AppendLine("");
        output.AppendLine("            return _itemList;");
    ...
    private void BuildMappedProcedureDataHandler()
    {
        ...
        output.AppendLine("        /// <param name=" + System.Convert.ToChar(34) + "parameters" + System.Convert.ToChar(34) + "></param>");
        output.AppendLine("        /// <returns></returns>");
        output.AppendLine("        internal virtual List<List<IDataItem>> Items(params object[] parameters)");
        output.AppendLine("        {");
        output.AppendLine("            _commandText = Constants.SQL_EXEC + GetFullDataEntityName();");
        output.AppendLine("            for (int i = 1; i < parameters.Length + 1; i++)");
        output.AppendLine("            {");
    ...
    private void BuildFunctionDataHandler()
    {
        ...
        output.AppendLine("    /// <param name=" + System.Convert.ToChar(34) + "parameters" + System.Convert.ToChar(34) + "></param>");
        output.AppendLine("    /// <returns></returns>");
        output.AppendLine("    public virtual List<IDataItem> Items(params object[] parameters)");
        output.AppendLine("    {");
        output.AppendLine("        try");
        output.AppendLine("        {");
        output.AppendLine("            _itemList.Clear();");
        output.AppendLine("            _commandText = Constants.SQL_SELECT + GetFullDataEntityName() + " + System.Convert.ToChar(34) + " (" + System.Convert.ToChar(34) + ";");
        output.AppendLine("            for (int i = 1; i < parameters.Length + 1; i++)");
        output.AppendLine("            {");
        output.AppendLine("                _commandText += base._parameterPrefix + i.ToString() + " + System.Convert.ToChar(34) + "," + System.Convert.ToChar(34) + ";");
        output.AppendLine("                _parameterizedValues.Add(new ParameterItemValue(base._parameterPrefix + i.ToString(), parameters[i - 1]));");
        output.AppendLine("            }");
        output.AppendLine("            _commandText = _commandText.Substring(0, _commandText.Length - 1) + " + System.Convert.ToChar(34) + ")" + System.Convert.ToChar(34) + ";");
        output.AppendLine("            _datareader = ExecuteReader(_commandText, _transaction);");
        output.AppendLine("            _itemList = MapDataReaderToDataItem(_datareader, _dataItem);");
        output.AppendLine("            return _itemList;");
        output.AppendLine("        }");
    ...
    private void BuildConfigurationHandler()
    {
        ...
        output.AppendLine("using System;");
        output.AppendLine("using System.Configuration;");
        output.AppendLine("internal static class Configuration");
        output.AppendLine("{");
        ...
        output.AppendLine("     }");
        output.AppendLine("     internal static String AdoNetCommandTimeout");
        output.AppendLine("     {");
        output.AppendLine("         get { return ConfigurationManager.AppSettings[Constants.ADONETCOMMANDTIMEOUT].ToString(); }");
        output.AppendLine("     }");
