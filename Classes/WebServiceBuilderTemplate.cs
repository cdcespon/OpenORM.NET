        ...
        private void BuildTablesServices(MyMeta.IDatabase db, GenerationProject generationProject)
        {
            foreach (MyMeta.ITable entity in db.Tables)
            {
                if (entity.Selected)
                {
                    ...
                    output.AppendLine("           Int64 userId = ValidateUserGetId();"); 
                    output.AppendLine("           try");
                    output.AppendLine("              {");
                    output.AppendLine("");
                    output.AppendLine("               if (userId != 0)");
                    output.AppendLine("               {");
                    ...
                    output.AppendLine("                             InitializeDto(InformationType.Ok, Newtonsoft.Json.JsonConvert.SerializeObject(entities.Items(" + fields + ")));");
                    output.AppendLine("                             break;");
                    ...
                    output.AppendLine("                    {");
                    output.AppendLine("                        InitializeDto(InformationType.InvalidSessionId, " + System.Convert.ToChar(34) + "InvalidSessionId" + System.Convert.ToChar(34) + ");");
                    output.AppendLine("                    }");
                    output.AppendLine("                }");
                    output.AppendLine("                catch (Exception ex)");
                    output.AppendLine("                {");
                    output.AppendLine("                    Log(LogType.Exception, ex.Message,(ex.InnerException == null ? " + System.Convert.ToChar(34) + "" + System.Convert.ToChar(34) + " : ex.InnerException.Source.ToString() + " + System.Convert.ToChar(34) + " - " + System.Convert.ToChar(34) + " + ex.InnerException.TargetSite.ToString() + " + System.Convert.ToChar(34) + " - " + System.Convert.ToChar(34) + " + ex.InnerException.Message) + " + System.Convert.ToChar(34) + " // " + System.Convert.ToChar(34) + " + ex.StackTrace, userId);");
                    output.AppendLine("                    InitializeDto(InformationType.Exception, ex.Message);");
                    output.AppendLine("                }");
                    output.AppendLine("                finally");
                    output.AppendLine("                {");
                    output.AppendLine("                    ReturnData();");
                    output.AppendLine("                }");
                    output.AppendLine("          }");
                    output.AppendLine("    /// <summary>");
                    output.AppendLine("    /// ");
                    ...
                    output.AppendLine("                     InitializeDto(InformationType.Ok, Newtonsoft.Json.JsonConvert.SerializeObject(entities.Add(" + fields + ")));");
                    output.AppendLine("                     break;");
                    output.AppendLine("                 case System.Net.WebRequestMethods.Http.Put:");
                    output.AppendLine("                     break;");
                    output.AppendLine("                         default:");
                    ...
                    output.AppendLine("                     InitializeDto(InformationType.Ok, Newtonsoft.Json.JsonConvert.SerializeObject(entities.Update(" + fields + ")));");
                    output.AppendLine("                    break;");
                    output.AppendLine("                case System.Net.WebRequestMethods.Http.Put:");
                    output.AppendLine("                    break;");
                    output.AppendLine("                         default:");
                    ...
                    output.AppendLine("                     InitializeDto(InformationType.Ok, Newtonsoft.Json.JsonConvert.SerializeObject(entities.Delete(" + fields + ")));");
                    output.AppendLine("                    break;");
                    output.AppendLine("                case System.Net.WebRequestMethods.Http.Put:");
                    output.AppendLine("                    break;");
                    output.AppendLine("                         default:");
                    ...
                    output.AppendLine("                    else");
                    output.AppendLine("                    {");
                    output.AppendLine("                        InitializeDto(InformationType.InvalidSessionId, GetErrorDescription((int)InformationType.InvalidSessionId));");
                    output.AppendLine("                    }");
                    output.AppendLine("                }");
                    output.AppendLine("                catch (Exception ex)");
                    output.AppendLine("                {");
                    ...
                    string relativepath = db.Name+ "/" + entity.Schema + "/"; 
                    SaveOutputToFile(db.Name+ "." + entity.Schema + "." + entity.Name+ ".asmx", relativepath,output, true);
                }
            }
        }
        /// <summary>
                ...
                if (entity.Selected)
                {
                    ...
                    foreach (MyMeta.IParameter item in entity.Parameters)
                    {
                        if (!item.Name.Equals("@RETURN_VALUE"))
                        {
                            ...
                            {
                                case "String":
                                    output.AppendLine("            " + item.LanguageType + " " + item.Name.Replace("@", "") + " = Context.Request.Form.GetValues(" + System.Convert.ToChar(34) + item.Name.Replace("@", "") + System.Convert.ToChar(34) + ") != null ? Context.Request.Form.GetValues(" + System.Convert.ToChar(34) + item.Name.Replace("@", "") + System.Convert.ToChar(34) + ")[0] : null;");
                                    break;

                                default:
                                    if (item.IsNullable)
                                        output.AppendLine("            " + item.LanguageType + "? " + item.Name.Replace("@", "") + " = Context.Request.Form.GetValues(" + System.Convert.ToChar(34) + item.Name + System.Convert.ToChar(34) + ") != null ? Convert.To" + item.LanguageType + "(Context.Request.Form.GetValues(" + System.Convert.ToChar(34) + item.Name + System.Convert.ToChar(34) + ")[0]) : new " + item.LanguageType + "?();");
                                    else
                                        output.AppendLine("            " + item.LanguageType + " " + item.Name.Replace("@", "") + " = Context.Request.Form.GetValues(" + System.Convert.ToChar(34) + item.Name + System.Convert.ToChar(34) + ") != null ? Convert.To" + item.LanguageType + "(Context.Request.Form.GetValues(" + System.Convert.ToChar(34) + item.Name + System.Convert.ToChar(34) + ")[0]) : new " + item.LanguageType + "();");
                                    break;
                            }
                            if (item.IsNullable && !item.LanguageType.Equals("String"))
                                parameters += "(" + item.LanguageType + ")" +  item.Name.Replace("@", "") + ",";
                            else
                    ...
                    output.AppendLine("                     InitializeDto(InformationType.Ok, Newtonsoft.Json.JsonConvert.SerializeObject(entities.Items(" + parameters + ")));");
                    output.AppendLine("                    break;");
                    output.AppendLine("                case System.Net.WebRequestMethods.Http.Put:");
                    output.AppendLine("                    break;");
                    output.AppendLine("                         default:");
     ...
     /// <summary>
        ...
        private void BuildMappedProceduresServices(MyMeta.IDatabase db, GenerationProject generationProject)
        {
            ...
            foreach (MappedStoredProcedure entity in _generationProject.MappedStoredProcedures)
            {
                    ...
                    string relativepath = db.Name+ "\\" + schemaName ; 
                    SaveOutputToFile(db.Name+ "." + entity.Name+ ".asmx", relativepath,output, true);
