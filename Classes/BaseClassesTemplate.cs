            ...
            if (entity.Selected)
            {
                ...
                foreach (MyMeta.IColumn column in entity.Columns)
                {
                    ...
                    if (column.IsInForeignKey && !column.IsInPrimaryKey)
                    {
                        ...
                        //foreach (MyMeta.IColumn pkColumn in column.ForeignKeys[0].PrimaryTable.PrimaryKeys)
                        //    args += "this._" + GetFormattedEntityName(pkColumn.ForeignKeys[0].Name) + ",";


                        //output.AppendLine("                             " + GetFormattedEntityName(column.Name) + " = new " + _namespace + ".Business.Relations." + GetSchemaName(column.ForeignKeys[0].PrimaryTable.Schema) + "." + column.ForeignKeys[0].PrimaryTable.Name + "().Items(" + args.Substring(0,args.Length -1) + ").FirstOrDefault();");
